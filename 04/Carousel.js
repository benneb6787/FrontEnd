...
var total = todayPhoto.length;
var now = 0;    //현재 page
var pass = 3    //보여질 이미지 수

function ImageList(ImageList, start, end) {
  var str = '';
  for(var i = start; i < end; i++){
    if(!ImageList[i]) break;
    str += '<img src="'+todayPhoto[i].img+'"> ';
  }
  wrap.innerHTML = str;
  pageCount.innerHTML = ((start / pass)+1) + '/' + (Math.floor(total/pass)+1);
}

preBtn.addEventListener('click', function(){
  if(now > 0) now = now - pass;
  ImageList(todayPhoto, now, now + pass);
});

nextBtn.addEventListener('click', function(){
  if(now <= (total/pass) + 1) now = now + pass;
  ImageList(todayPhoto, now, now + pass);
});
